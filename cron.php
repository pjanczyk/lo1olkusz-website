...
syslog(LOG_INFO, "[cron.php] refeshing...");
...
if (updateFile($lnPath, $lnJson)) {
    syslog(LOG_INFO, "UPDATED {$lnPath}");
}
else {
    syslog(LOG_INFO, "up-to-date {$lnPath}");
}
if (updateFile($replsPath, $replsJson)) {
    syslog(LOG_INFO, "UPDATED {$replsPath}");
}
else {
    syslog(LOG_INFO, "up-to-date {$replsPath}");
}
